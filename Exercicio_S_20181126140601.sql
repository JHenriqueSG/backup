...
create view qtcompras as
select cliente.chave_cliente, cliente.nome as cliente,count(*) as quantidade from cliente
join compra on cliente.chave_cliente = compra.cliente
group by chave_cliente, cliente.nome

 select * from qtcompras

--2)Faça uma view que retorne os 3 produtos mais caros, ordenados do mais caro para o mais barato

create view preço_produto as 
select top 3 * from produto
order by preco desc

select * from preço_produto
...
create function Com_quantidade(@id_compra int)
returns table
as return ( select produto.descricao, itens_compra.quantidade from itens_compra
join produto on produto.chave_produto = itens_compra.produto
where itens_compra.compra = @id_compra)

select * from Com_quantidade(200)-- tem que adicionar o parametro

--4)Faça uma função que dado um id de compra, recebido por parâmetro,  retorne a quantidade de produtos que foram comprados

create function qt_prod(@id_compra int)
returns int as
begin
declare @qtd int
select @qtd = sum(quantidade) from itens_compra where compra = @id_compra
return @qtd
end

select compra.codigo_nota, dbo.qt_prod(1)
