/*1) Criar as tabelas e suas dependências (Chave estrangerias)*/
...
Endereço VARCHAR(50), DataNasc DATE, FK_ID_Superv DECIMAL(4,0), FK_ID_Depto DECIMAL(2,0),
...
/*2) Inserir 3 departamentos, sendo eles (construção, computação e pesquisa).*/
...
/*30  Inserir 5 funcionários*/
insert into funcionario(ID_Func, NomeFunc, Endereço, DataNasc, Sexo, Salario, FK_ID_Depto)
values (1, 'Maria', 'jardim 1', '1987-07-03', 'F', 3000.00, 3)
insert into funcionario(ID_Func, NomeFunc, Endereço, DataNasc, Sexo, Salario, FK_ID_Depto)
values (2, 'João', 'jardim 2', '1980-12-10', 'M', 3000.00, 2)
insert into funcionario(ID_Func, NomeFunc, Endereço, DataNasc, Sexo, Salario, FK_ID_Depto)
...
/*4) Inserir 10 diferentes relações de trabalho projeto (tabela trabalha)*/
insert into projeto (ID_proj, NomeProj, Localização, FK_ID_Depto)
...
values (5, 5, 'josé', '1990-01-01', 'M', 'conjuge')
insert into dependente (ID_Dep, FK_ID_Func, NomeDep, DataNasc, Sexo, Parentesco)
...
/*6) Listar nome dos Gerentes dos projetos que estão sob a gerência do departamento construção*/
update departamento
set ID_Gerente = 4
where ID_Depto = 1
...
/*7) Nome de todos os funcionários que trabalham em um projeto sob a gerência do departamento
contrução. (Usar subquery)*/
select NomeFunc from funcionario
where  FK_ID_Depto in(select ID_Depto from departamento where ID_Depto in
(select FK_ID_Depto from projeto where NomeDepto = 'construção'))

/*8) Contabilizar total de dependentes dos funcionários do departamento pesquisa.(subquery)*/
...
/*9) Contabilizar o número de funcionários por projeto (join)*/
